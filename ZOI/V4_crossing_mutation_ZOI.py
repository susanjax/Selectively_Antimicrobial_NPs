...
mutation_rate = 0.3
cross_over_rate = 0.3
df = V4_ga_compd_generation_ZOI.fitness(V4_ga_compd_generation_ZOI.population(population_size)).sort_values('Fitness', ascending=False)
...
def evolve_crossing(df_compound_list, cross_over_rate, mutation_rate):
    ...
    while i < len(dff):
        ...
        cross_individual = V4_cross_modified_ZOI.to_crossover(individual1, individual2, cross_over_rate)
        mutate_individual = V4_cross_modified_ZOI.to_mutation(cross_individual, mutation_rate)
    ...
    dframe_evolved = V4_ga_compd_generation_ZOI.fitness(dframe_copy)
    # print('dframe_evolved', dframe_evolved)
...
# print('here',evolve_crossing(df, cross_over_rate, mutation_rate), evolve_crossing(df, cross_over_rate, mutation_rate).columns)